			...
			targets.map(async (target) => {
				...
				for (const user of exceeded) {
					...
					await sendServerThresholdNotifications(target.organizationId, {
						...
						Threshold: 100,
						Message: `Host UID ${user.uid} holds ${user.currentInstances} inotify instances across ${user.descriptorReferences} descriptors (limit ${usage.maxInstances}). At the limit, new file watchers can fail, disrupting log collection or app startup. Existing watchers usually keep running.`,
