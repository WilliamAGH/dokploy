...
export const composeRouter = createTRPCRouter({
	...
	update: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const { expectedComposeFile, ...composeData } = input;
			const updated =
				expectedComposeFile !== undefined && input.composeFile !== undefined
					? await updateComposeFileIfUnchanged(
							input.composeId,
							expectedComposeFile,
							input.composeFile,
						)
					: await updateCompose(input.composeId, composeData);
			if (!updated) {
				throw new TRPCError({
					code: "CONFLICT",
					message: "Compose file changed while you were editing it",
				});
			}
			await audit(ctx, {
				action: "update",
	...
	addComposeVolume: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			if (!composeData.services?.[input.serviceName]) {
				throw new TRPCError({
					code: "NOT_FOUND",
					message: "Compose service not found",
				});
			}
			const updatedComposeFile = addVolumeToComposeFile(
				compose.composeFile,
				input.serviceName,
				`${input.source}:${input.target}`,
			);
			const updated = await updateComposeFileIfUnchanged(
				input.composeId,
				compose.composeFile,
				updatedComposeFile,
			);
			if (!updated) {
				throw new TRPCError({
					code: "CONFLICT",
					message: "Compose file changed while adding the volume",
				});
			}
			await audit(ctx, {
				action: "update",
	...
	updateComposeVolume: protectedProcedure
		.input(
			apiFindCompose.extend({
				serviceName: z.string().min(1),
		...
		.mutation(async ({ input, ctx }) => {
			...
			const existingVolume = extractServiceVolumes(composeData).find(
				(volume) =>
					volume.serviceName === input.serviceName &&
					volume.target === input.originalTarget,
			);
			if (!existingVolume?.source || !existingVolume.target) {
				throw new TRPCError({
					code: "NOT_FOUND",
					message: "Compose volume not found",
				});
			}
			const updatedComposeFile = updateVolumeInComposeFile(
				compose.composeFile,
				input.serviceName,
			...
			if (!updated) {
				throw new TRPCError({
					code: "CONFLICT",
					message: "Compose file changed while updating the volume",
				});
			}
			await audit(ctx, {
				action: "update",
	...
	removeComposeVolume: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const updatedComposeFile = removeVolumeFromComposeFile(
				compose.composeFile,
				input.serviceName,
				input.target,
			);
			...
			if (!updated) {
				throw new TRPCError({
					code: "CONFLICT",
					message: "Compose file changed while removing the volume",
				});
			}
			await audit(ctx, {
				action: "update",
